...
const JobDescription = () => {
    ...
    const isClosed = singleJob?.deadline ? new Date(singleJob.deadline) < new Date() : false;
    const formattedDeadline = singleJob?.deadline
    ...
    const applyJobHandler = async () => {
       try {
          if (isClosed) {
             toast.error("This job is closed because the deadline has passed.");
             return;
          }
          if (!user?._id) {
            ...
            <div className='mx-auto my-10 max-w-4xl px-4'>
                <div className='bg-white rounded-lg shadow-md p-6 md:p-8'>
                    <div className='flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6'>
                        ...
                        <Button
                            onClick={!isApplied && !isClosed ? applyJobHandler : null}
                            disabled={isApplied || isClosed}
                            className={`rounded-lg px-6 py-2 ${(isApplied || isClosed) ? 'bg-gray-600 cursor-not-allowed' : 'bg-[#7209b7] hover:bg-[#5f32ad]'}`}>
                            {isClosed ? 'Closed' : isApplied ? 'Already Applied' : 'Apply Now'}
                        </Button>
                    </div> 
                        ...
                        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                            ...
                            <div className='bg-gray-50 p-4 rounded-lg'>
                                ...
                                <p className='text-gray-700'>
                                    {formattedDeadline} {isClosed ? '(Closed)' : '(Open)'}
                                </p>
                            </div>
                        </div>
